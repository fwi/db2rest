...
public class RestTableQueries {
    ...
    public List<?> insert(List<Map<String, Object>> records) {
        ...
        // There is no reliable way to get generated values back when using a batch-update.
    ...
    public String insertQuery(List<String> columns) {
        ...
        // Use quotes for names anyway in case a name is a SQL-reserved word (e.g. "user").
    ...
    public String updateQuery(Map<String, Object> params, boolean allowUpdateAll) {
        ...
        for (var column: params.keySet()) {
            if (selectOnlyColumns.contains(column)
                    || column.startsWith(DB_QUERY_PREFIX)) {
                continue;
            }
            if (first) {
            	first = false;
    ...
    public List<?> delete(List<Map<String, Object>> records) {
        ...
        int deletedRows = transactionTemplate.execute((status) -> {
            ...
            for (var params : records) {
                if (params.isEmpty()) {
                    throw new BadRequestException("Selection to delete is required (no selection parameters provided).");
                }
